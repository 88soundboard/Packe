...
/// <summary>
/// US (ANSI 104-key) レイアウト。JISと物理的に異なる部分（Enterの形状、
/// バックスラッシュの位置、スペースバーの長さ、無変換/変換/かな の不在など）
/// を反映した独自の座標定義。
/// </summary>
public static class UsLayout
{
    ...
    [
        ...
        new("Minus",        "- _",    11f,   1f,    1f),
        new("Equal",        "= +",    12f,   1f,    1f),
        new("Backspace",    "BACKSPACE", 13f, 2.5f,  1f),
        ...
        new("Backslash",    "\\ |",   13.5f, 2f,    2f),

        // ── ASDF行 ──────────────────────────────────────────
        new("CapsLock",     "CAPS",   0f,    1.75f, 3f),
        ...
        new("Enter",        "ENTER",  12.75f,2.25f, 3f),
        ...
        new("ShiftRight",   "SHIFT",  12.25f,2.75f, 4f),

        // ── スペース行（US配列は無変換/変換/かなが無く、スペースが長い）──
        new("ControlLeft",  "CTRL",   0f,    1.25f, 5f),
        new("MetaLeft",     "WIN",    1.25f, 1.25f, 5f),
        new("AltLeft",      "ALT",    2.5f,  1.25f, 5f),
        new("Space",        "SPACE",  3.75f, 6.25f, 5f),
        new("AltRight",     "ALT",    10f,   1.25f, 5f),
        new("ControlRight", "CTRL",   11.25f,1.25f, 5f),
